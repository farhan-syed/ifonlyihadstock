            ...
            <div className="flex-none pt-5 pb-5">
                <div className="flex flex-col items-center">
                    <div className={`mt-0 w-80 px-6 py-6 bg-white rounded-lg shadow-2xl border-b-8 ${returnGainBorderColor()}`}>
                        ...
                        <Link className="text-green-500 inline-flex text-2xl font-main font-bold" to={`/ticker/${props.results.symbol}`}>{props.results.companyName}<svg className="h-full" height="20" width="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        <p className="text-center font-semibold text-sm"> ≈ {formatToDollar(props.results.sellPrice)}/share</p>
                        ...
                        <div className="text-center">
                            <a href={`https://twitter.com/intent/tweet?text=If I'd invested ${formatToDollar(props.results.amount)} into ${props.results.symbol} on ${props.results.purchaseDate}. I'd have ${formatToDollar(props.results.value)} if I had sold on ${props.results.sellDate}. Find out for yourself:&url=http://ifonlyihadstock.com&hashtags=ifonlyihadstock`} className="mt-4 inline-flex items-center space-x-2 text-white bg-twitter-blue text-sm font-bold p-2 rounded-sm">
